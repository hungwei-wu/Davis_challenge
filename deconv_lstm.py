...
def init_state(inputs,
  ...
  if inputs is not None:
    ...
    inferred_batch_size = inputs.get_shape().with_rank_at_least(1).as_list()[0]
    batch_size = tf.shape(inputs)[0]
    dtype = inputs.dtype
  else:
    inferred_batch_size = 0
    batch_size = 0
  final_shape=[inferred_batch_size] + state_shape
  #final_shape=[inferred_batch_size] + state_shape
  
  #init_shape=tf.stack(final_shape)
  init_shape=final_shape
  
  initial_state = state_initializer(init_shape,dtype=dtype)
  #initial_state=tf.Variable(tf.zeros(init_shape), name="state")
...
def deconv_lstm_cell(inputs,
  ...
  *this is an upsampling lstm cell. stride=upscale, and kernel size= 2*stride
  ...
  if state is None:
    state_shape=[spatial_size[0]*upscale, spatial_size[1]*upscale, 2 * num_channels]
    state = init_state(inputs, state_shape)
  with tf.variable_scope(scope,
    ...
    #subsample
    h=layers.conv2d(h,num_channels, [upscale*2,upscale*2], stride=upscale)
    ...
    i_j_f_o = layers.conv2d_transpose(inputs_h,
                            4 * num_channels, [upscale*2, upscale*2],
                            stride=upscale,
                            activation_fn=None,
                            scope='Gates')
